...
MODES = ["BASELINE", "MICRO_V1", "MICRO_V2", "MICRO_V3"]
...
def run_benchmark():
    ...
    for mode in MODES:
        ...
        llm_o_en = {k: v for k, v in ov_en.items() if "forced" in str(v.get("override", ""))}
        llm_o_fr = {k: v for k, v in ov_fr.items() if "forced" in str(v.get("override", ""))}
