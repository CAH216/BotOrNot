...
MODES = ["BASELINE (No_LLM)", "PASSIVE", "ACTIVE_FP", "ACTIVE_FN", "ACTIVE_COMBINED"]
...
def run_benchmark():
    ...
    evs_tr_en = [load_event(x) for x in [1, 3, 30]]
    ...
    evs_tr_fr = [load_event(x) for x in [2, 4, 31]]
    ...
    for mode in MODES:
        ...
        args_en = {
            ...
            "arbitration_mode": mode.replace("BASELINE (No_LLM)", "PASSIVE")
        }
        ...
        llm_o_en = {k: v for k, v in ov_en.items() if "groq_" in str(v.get("override", ""))}
        llm_o_fr = {k: v for k, v in ov_fr.items() if "groq_" in str(v.get("override", ""))}
        ...
        print(f"EN Score: {results[mode]['EN']} | FR Score: {results[mode]['FR']}")
        print(f"Overrides (EN): {len(llm_o_en)} | Overrides (FR): {len(llm_o_fr)}")
    ...
    for m, r in results.items():
        ...
        if r['Overrides_EN'] > 0 or r['Overrides_FR'] > 0:
            ...
            for lang, dets in [("EN", r['Detail_EN']), ("FR", r['Detail_FR'])]:
                for uid, o in dets.items():
                    print(f"     {lang} Muted {uid}: {o['override']}")
